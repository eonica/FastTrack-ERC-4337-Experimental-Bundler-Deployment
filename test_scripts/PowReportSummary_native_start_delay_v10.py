#!/usr/bin/env python3
"""Extract and summarize SmartWatts data from a .tar.gz or .tgz archive.

The script uses only the Python standard library and is suitable for
Python 3.8+ on Windows, Linux, and macOS.
"""
...
SCRIPT_VERSION = "2026-07-30-native-v10-start-delay"
...
def format_number(value: float) -> str:
    """Write numbers without unnecessary trailing zeroes."""
...
def find_exactly_one(paths: Iterable[Path], description: str) -> Path:
    ...
    if len(unique) != 1:
        ...
        raise ValueError(
            f"Expected exactly one {description}, but found {len(unique)}.{suffix}"
        )
    return unique[0]
...
def has_ancestor(path: Path, ancestor_name: str, stop_at: Path) -> bool:
    """Return True when path has the named ancestor below stop_at."""
    target = ancestor_name.lower()
    stop_resolved = stop_at.resolve()
    current = path.resolve()

    while current != stop_resolved and current != current.parent:
        if current.name.lower() == target:
            return True
        current = current.parent
    return current.name.lower() == target


def find_scope_power_report(swatts_directory: Path, scope_name: str) -> Path:
    """Find user.slice/<scope_name>/PowerReport.csv below a sensor-* directory."""
    matches: List[Path] = []

    for report in recursive_files(swatts_directory, "PowerReport.csv"):
        if report.parent.name.lower() != scope_name.lower():
            continue
        if report.parent.parent.name.lower() != "user.slice":
            continue

        relative_parts = [part.lower() for part in report.relative_to(swatts_directory).parts]
        if not any(
            part.startswith("sensor-") and part not in {"sensor-rapl", "sensor-global"}
    ...
    return find_exactly_one(
        matches,
        f"PowerReport.csv under sensor-*/user.slice/{scope_name}",
    )
...
def normalize_power_report(
    ...
    """Keep timestamp, power, and scope, then sort by scope and timestamp."""
    allowed_scopes = (
        None
...
def calculate_interval_result(
    ...
    if use_all_values:
        # Use the complete available series for this scope. The first sample is
        # necessarily PRESTART because the requested formula weights each power
        # value by the elapsed time since the preceding timestamp.
        if len(scope_rows) < 2:
            raise ValueError(
        ...
        # START: first sample at or after the experiment start.
        start_index = next(
            (
                index
        ...
        # STOP: last sample at or before the experiment stop.
        stop_index = next(
            (
                index
        ...
        # The weighted formula needs the timestamp immediately preceding START.
        # If the first matching sample is the first row for this scope, reserve it
        # as PRESTART and move START to the following sample.
        if start_index == 0:
...
def process_sensor_output(
    ...
    anvil_source = find_scope_power_report(swatts_directory, "anvil.scope")
    alto_source = find_scope_power_report(swatts_directory, "alto.scope")
...
def extraction_folder_for(archive_path: Path) -> Path:
    ...
    if lower_name.endswith(".tar.gz"):
        ...
        raise ValueError(
            f"Input file must have a .tar.gz or .tgz extension: {archive_path}"
        )

    if not folder_name.strip():
        raise ValueError(
            f"Cannot determine an extraction-folder name from: {archive_path}"
        )
    return archive_path.parent / folder_name


def safe_extract_tar(archive_path: Path, destination: Path) -> None:
    """Extract a gzip-compressed TAR while rejecting unsafe paths and links."""
    ...
    with tarfile.open(archive_path, mode="r:gz") as archive:
        ...
        for member in members:
            ...
            if member.issym() or member.islnk():
                ...
                try:
                    ...
                    raise ValueError(
                        f"Unsafe archive link target in {member.name}: "
                        f"{member.linkname}"
                    ) from exc
        ...
        try:
            ...
            archive.extractall(
                destination, members=members, filter="fully_trusted"
            )
        except TypeError:
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Extract a sensor .tar.gz/.tgz archive, normalize four PowerReport.csv files, "
            "and calculate weighted-average power for CPU and, unless --nodram is used, DRAM."
        )
    )
    parser.add_argument(
        "archive_file",
        type=Path,
        help="Path to the .tar.gz or .tgz archive",
    )
    parser.add_argument(
        "--force",
        action="store_true",
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        # Normal dot-decimal parsing still works if the OS locale is unavailable.
        pass
    ...
    if extract_root.exists():
        if not args.force:
            raise FileExistsError(
                f"Extraction folder already exists: {extract_root}. "
    ...
    if not sensor_output_directories:
        raise ValueError(
            f"No sensor_output directory was found after extracting {archive_path}."
        )
